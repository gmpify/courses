...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        elif type(self.ant) is QueenAnt and not self.ant.impostor:
            return
        elif type(self.ant) is BodyguardAnt and self.ant.ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place:
            ...
            if bee and place is not hive and current_range >= self.min_range and current_range <= self.max_range:
                return bee
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        self.impostor = self.number_of_queens > 0
        QueenAnt.number_of_queens += 1
        self.doubled_ants = [self]

    def double_ant(self, place):
        if place.ant and place.ant not in self.doubled_ants:
            self.doubled_ants.append(place.ant)
            place.ant.damage *= 2
        if place.ant and place.ant.container and place.ant.ant and place.ant.ant not in self.doubled_ants:
    ...
    def action(self, colony):
        ...
        entrance = self.place
        exit = self.place
        while entrance or exit:
            if entrance:
                self.double_ant(entrance)
                entrance = entrance.entrance
            if exit:
                self.double_ant(exit)
                exit = exit.exit
