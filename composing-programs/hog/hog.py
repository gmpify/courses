...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < goal and opponent_score < goal:
        ...
        if 2 * score == opponent_score or 2 * opponent_score == score:
            score, opponent_score = opponent_score, score

        who = other(who)
    return score, opponent_score  # You may wish to change this line.

def play1(strategy0, strategy1, goal=GOAL_SCORE):
    """Simulate a game and return the final scores of both players, with
    Player 0's score first, and Player 1's score second.
    ...
    while max(scores) < goal:
        ...
        if 2 * scores[who] == scores[other(who)] or 2 * scores[other(who)] == scores[who]:
            scores[who], scores[other(who)] = scores[other(who)], scores[who]

        who = other(who)
    return tuple(scores)  # You may wish to change this line.
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    max_average = 0
    while (k <= 10):
        average = make_averaged(roll_dice, 2000)(k, dice)
        print(k, 'dice scores', average, 'on average')
        if average > max_average:
            max_average, max_num_rolls = average, k
        k += 1
...
def swap_strategy(score, opponent_score):
    ...
    5
    """
    zero_dice_score = max(opponent_score // 10, opponent_score % 10) + 1
    if 2 * (score + zero_dice_score) == opponent_score:
        return 0
    if 2 * opponent_score == score + zero_dice_score:
        return BASELINE_NUM_ROLLS
    if max(opponent_score // 10, opponent_score % 10) + 1 >= BACON_MARGIN:
        return 0
    return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if (score + zero_dice_score + opponent_score) % 7 == 0:
        return 0

    if (score + opponent_score) % 7 == 0: #four-sided dice
