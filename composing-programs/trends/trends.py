...
def extract_words(text):
    ...
    ['cat', 'on', 'my', 'keyboard']
    """
    chars = [x if x in ascii_letters else ' ' for x in text]
    escaped_text = ''.join(chars)
    return escaped_text.split()
...
def find_centroid(polygon):
    ...
    (1.0, 2.0, 0.0)
    """
    shoelace_sum, centroid_lat, centroid_lon = 0, 0, 0

    for k in range(len(polygon) - 1):
        lat, lon = latitude(polygon[k]), longitude(polygon[k])
        lat_next, lon_next = latitude(polygon[k + 1]), longitude(polygon[k + 1])
        sum_term = lat * lon_next - lat_next * lon
        shoelace_sum += sum_term
        centroid_lat += (lat + lat_next) * sum_term
        centroid_lon += (lon + lon_next) * sum_term

    area = 0.5 * shoelace_sum

    if area:
        centroid_lon = centroid_lon / (6 * area)
        centroid_lat = centroid_lat / (6 * area)
    else:
        centroid_lat, centroid_lon = latitude(polygon[0]), longitude(polygon[0])
...
def find_state_center(polygons):
    ...
    for polygon in polygons:
        ...
        sum_lat += centroid_lat * area
        sum_lon += centroid_lon * area
        sum_area += area

    state_center_lat = sum_lat / sum_area
    state_center_lon = sum_lon / sum_area
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        ...
        for state, state_center in us_centers.items():
            distance = geo_distance(position_tweet, state_center)
...
def average_sentiments(tweets_by_state):
    ...
    for state, tweets in tweets_by_state.items():
        ...
        tweet_sentiment_values = [sentiment_value(sentiment) for sentiment in tweet_sentiments if has_sentiment(sentiment)]
        if len(tweet_sentiment_values) > 0:
            averaged_state_sentiments[state] = sum(tweet_sentiment_values) / len(tweet_sentiment_values)
